...
4. `보내기` — Premiere XML / SRT / CSV, 공유 시트 또는 파일 다운로드로 전달

## 프리미어 연동

- **Premiere XML (.xml)**: FCP7 XMEML v4 형식으로 테이크를 구간별 클립으로 나눕니다 (`T01.1 삭제`, `T01.2 편집에쓰자`…).
  가져온 뒤 `/RELINK/` 오프라인 참조를 실제 카메라 파일에 연결해야 합니다. 파일명만으로 자동 연결을 보장하지 않습니다.
  1080p 시퀀스에 비디오 1트랙과 연결된 스테레오 오디오 2트랙을 만듭니다. 설정한 프레임레이트를 카메라와 맞춰 주세요.
  삭제 판정 구간도 유지하며 시퀀스에 `삭제 구간` 범위 마커와 메모 마커를 넣습니다.
- **싱크 오프셋**: 플래시(앱 0초)는 카메라 파일 안 δ초 지점에 찍힘. 프리미어에서 플래시 프레임 위치를 보고
  ...
  음수 오프셋으로 카메라 녹화 전 구간이 생기면 그만큼 타임라인에 빈 공간을 남기고 실제 존재하는 소스 구간부터 배치합니다.
- **마이크 녹음** (설정에서 켬): 롤 동안 폰 마이크로 오디오 녹음 → `테이크 오디오 보내기`로 테이크별
  ...
  CUT 판정 대기 시간도 오디오에는 남습니다. 로그는 CUT 시각에서 끝나므로 오디오가 더 길 수 있습니다. 취소하면 기존 녹음을 끊지 않고 이어갑니다.
- **SRT**: 메모를 자막으로. XML과 같은 프레임 누적 타임라인 기준이며 범위 밖 메모는 제외하고 동일 시각 메모는 한 자막으로 묶습니다.
- **CSV**: 구간/메모 포함 플랫 로그.
...
| `export-xml.js` | 정수 프레임과 V/A 링크를 사용하는 FCP7 XML 생성 |
| `style.css` | 다크 모바일 UI |
| `manifest.webmanifest`, `sw.js`, `icon*` | PWA (오프라인 캐시, 홈화면 설치) |

개발 검증: `npm ci`, `npm test`, `npm run check`. DOM/마이크 대역과 XML 구조를 검사하며 실제 Premiere 가져오기와 iOS/Android 공유·마이크 실기는 별도로 확인해야 합니다.
