# offcut — 촬영 로거 (take logger)

소규모 촬영(인터뷰 등)에서 감독/조연출이 폰으로 **롤 타임코드 + 구간 판정 + 메모**를 기록하고,
결과를 **프리미어 프로로 바로 넘기는** PWA.

배포: https://offcut-xi.vercel.app (Vercel, `vercel --prod --name offcut`)

## 사용 흐름

1. `ROLL` — 화면 플래시 + 1kHz 삑 (슬레이트 대용: 영상 첫 프레임 = 앱 타임라인 0초)
2. 롤 도중 `채택 / 보류 / 버림` — "마지막 마크 ~ 지금" 구간을 판정하고 새 구간 시작 (카메라 안 끊음)
   - `↩ 직전 구간 판정 취소` 있음
...
- **FCPXML**: 테이크를 구간별 `asset-clip`으로 쪼개서보냄 (`T01.1 버림`, `T01.2 채택`…).
  같은 카메라 파일의 다른 소스 오프셋을 참조 → 릴링크하면 실화면 자동 연결.
  "버림" 구간엔 전체 길이 `버림 - 삭제` 마커, 메모는 포인트 마커.
...
vercel --prod --name offcut   # 배포 (폴더명에 한글 있어서 --name 필수)
```
